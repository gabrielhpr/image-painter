/*
  ...
  DESENVOLVIDAS E IMPLEMENTADAS POR MIM SEGUINDO AS INTRUÇÕES DESSE EP
  ...
  Referências: Com excessão das rotinas fornecidas no esqueleto e em sala
...
int main(int argc, char *argv[]){
	...
	char nomeDoArquivoImagem[MAX_NOME];
	int i=0;
	//Tamanho da string
	int n;
	...
	n = strlen(argv[1]);
	while(i < n){
		nomeDoArquivoImagem[i] = argv[1][i];
		i++;
	}
	nomeDoArquivoImagem[i] = '\0';
	/* 2. carregue de uma arquivo, no formato PPM, a imagem original */
	imgOriginal = carregueImagemPPM(nomeDoArquivoImagem);
...
*/
void quit(Imagem *tela, Imagem *img, CelRegiao *iniRegioes){
	freeImagem(tela);
	freeImagem(img);
...
*/
void graveImagem(Imagem *img){
	char nomeDoArquivo[MAX_NOME];
	int i=0;
	//Recebe um caractere
	int c;
	printf("Digite o nome do arquivo em que quer salvar: ");
	while((c=getchar()) != '\n' && c != EOF){
		nomeDoArquivo[i] = c;
		i++;
	}
	nomeDoArquivo[i] = '\0';
	graveImagemPPM(nomeDoArquivo, img);
}
